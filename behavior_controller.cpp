...
private:
    ...
    std::string gap_key_char;
    std::string pot_key_char;
    std::string pure_key_char;
...
public:
    BehaviorController() {
        ...
        n.getParam("gap_mux_idx", gap_mux_idx);
        n.getParam("pot_mux_idx", pot_mux_idx);
        n.getParam("pure_mux_idx", pure_mux_idx);
        // ***Add mux index for new planner here***
        ...
        n.getParam("gap_button_idx", gap_button_idx);
        n.getParam("pot_button_idx", pot_button_idx);
        n.getParam("pure_button_idx", pure_button_idx);
        // ***Add button index for new planner here***
        ...
        n.getParam("gap_key_char", gap_key_char);
        n.getParam("pot_key_char", pot_key_char);
        n.getParam("pure_key_char", pure_key_char);
        // ***Add key char for new planner here***
    ...
    void joy_callback(const sensor_msgs::Joy & msg) {
        ...
        } else if (msg.buttons[AEB_button_idx]){
            toggle_mux(AEB_mux_idx, "AEB Planner");
        } else if (msg.buttons[wall_button_idx]){
            toggle_mux(wall_mux_idx, "Wall Following");
        } else if(msg.buttons[gap_button_idx]){
            toggle_mux(gap_mux_idx, "Gap Follow");
        } else if(msg.buttons[pot_button_idx]){
            toggle_mux(pot_mux_idx, "Potential Field");
        } else if(msg.buttons[pure_button_idx]){
            toggle_mux(pure_mux_idx, "Pure Pursuit");
        }
    ...
    void key_callback(const std_msgs::String & msg) {
        ...
        if (msg.data == joy_key_char) {
            ...
            toggle_mux(wall_mux_idx, "Wall Following");
        } else if(msg.data == gap_key_char){
            toggle_mux(gap_mux_idx, "Gap Follow");
        } else if(msg.data == pot_key_char){
            toggle_mux(pot_mux_idx, "Potential Field");
        } else if(msg.data == pure_key_char){
            toggle_mux(pure_mux_idx, "Pure Pursuit");
        }
        // ***Add new else if statement here for new planning method***
